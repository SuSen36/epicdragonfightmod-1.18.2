...
import susen36.epicdragonfight.world.capabilities.entitypatch.enderdragon.DragonFlyingPhase;
...
import susen36.epicdragonfight.world.capabilities.ai.CombatBehaviors.Behavior;
import susen36.epicdragonfight.world.capabilities.ai.CombatBehaviors.BehaviorSeries;
import susen36.epicdragonfight.world.capabilities.ai.CombatBehaviors.Health.Comparator;
	...
	public static final CombatBehaviors.Builder<EnderDragonPatch> ENDER_DRAGON = CombatBehaviors.<EnderDragonPatch>builder()
		.newBehaviorSeries(
			...
			BehaviorSeries.<EnderDragonPatch>builder().weight(1000.0F).cooldown(0).canBeInterrupted(false).looping(false)
				.nextBehavior(Behavior.<EnderDragonPatch>builder().health(0.4F, Comparator.LESS_RATIO).custom((mobpatch) -> mobpatch.getOriginal().getDragonFight() != null && mobpatch.getOriginal().getDragonFight().getCrystalsAlive() > 0)
				.behavior((mobpatch) -> {
			...
			BehaviorSeries.<EnderDragonPatch>builder().weight(10.0F).cooldown(1600).canBeInterrupted(false).looping(false)
				.nextBehavior(Behavior.<EnderDragonPatch>builder().health(0.5F, Comparator.LESS_RATIO).custom((mobpatch) -> mobpatch.getOriginal().getDragonFight() != null && mobpatch.getOriginal().getDragonFight().getCrystalsAlive() > 0)
				.behavior((mobpatch) -> {
