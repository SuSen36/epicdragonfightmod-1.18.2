	...
	public static final CombatBehaviors.Builder<IDragonPatch> ENDER_DRAGON = CombatBehaviors.builder()
		.newBehaviorSeries(
			BehaviorSeries.builder().weight(50.0F).canBeInterrupted(false).looping(false)
				.nextBehavior(Behavior.builder().randomAnimationBehavior(Animations.DRAGON_LEFT_TAIL_SWEEP, Animations.DRAGON_RIGHT_TAIL_SWEEP).randomChance(0.1F).withinDistance(0.0D, 7.0D).withinAngle(0.0F, 60.0F))
				.nextBehavior(Behavior.builder().animationBehavior(Animations.DRAGON_ATTACK2).withinDistance(0.0D, 7.0D))
				.nextBehavior(Behavior.builder().animationBehavior(Animations.DRAGON_ATTACK1))
		).newBehaviorSeries(
			BehaviorSeries.builder().weight(50.0F).canBeInterrupted(false).looping(false)
				.nextBehavior(Behavior.builder().animationBehavior(Animations.DRAGON_ATTACK1).withinDistance(0.0D, 5.0D).withinAngle(0.0F, 60.0F))
