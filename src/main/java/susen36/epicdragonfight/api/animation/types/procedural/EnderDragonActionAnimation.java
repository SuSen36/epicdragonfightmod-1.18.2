...
public class EnderDragonActionAnimation extends ActionAnimation implements ProceduralAnimation {
	...
	@Override
	public void tick(IDragonPatch enderdragonpatch) {
		super.tick(enderdragonpatch);

		Vec3 entitypos = enderdragonpatch.getOriginal().position();
		OpenMatrix4f toWorld = OpenMatrix4f.mul(OpenMatrix4f.createTranslation((float)entitypos.x, (float)entitypos.y, (float)entitypos.z), enderdragonpatch.getModelMatrix(1.0F), null);
		float elapsedTime = enderdragonpatch.getAnimator().getPlayerFor(this).getElapsedTime();
	...
	@Override
	@OnlyIn(Dist.CLIENT)
	public void renderDebugging(PoseStack poseStack, MultiBufferSource buffer, IDragonPatch enderdragonpatch, float playTime, float partialTicks) {
		OpenMatrix4f modelmat = enderdragonpatch.getModelMatrix(partialTicks);
		LivingEntity originalEntity = enderdragonpatch.getOriginal();
